...
class Trainer:
    ...
    def __init__(
        ...
        # newly added
        self.eval_res = []
        self.patience = self.model.config.early_stopping_patience  # early stop if no better results
        self.n_worse_res = 0
    ...
    def exit_handler(self):
        # NB. uncomment this to del model folder after exit
        # print('[exit_handler] del model folder to save space')
        # shutil.rmtree(f"{Path().resolve()}/model_output/{self.args.run_name}")
        pass
    ...
    def _maybe_log_save_evaluate(self, tr_loss, model, trial, epoch):
        ...
        if metrics is not None and 'early_stop' in metrics:
    ...
    def evaluate(
        ...
        if output.metrics is not None:
            ...
            metric_value = output.metrics[metric_to_check]
            operator = np.greater
            if (
                    self.state.best_metric_outside is None
                    or operator(metric_value, self.state.best_metric_outside)
            ):
                self.state.best_metric_outside = metric_value
                self.state.best_metric_outside_epoch = self.state.epoch
                self.state.best_metric_outside_steps = self.state.global_step
                self.n_worse_res = 0
                output.metrics['is_best'] = True
            else:
                self.n_worse_res += 1
                output.metrics['is_best'] = False
            print(f'best {metric_to_check}: {self.state.best_metric_outside:.4f}'
                  f'@epoch{self.state.best_metric_outside_epoch} global_step{self.state.best_metric_outside_steps}')
            ...
            best_res = {metric_to_check: self.state.best_metric_outside, 'epoch': self.state.best_metric_outside_epoch,
                        'global_step': self.state.best_metric_outside_steps}
            self.eval_res.append({'cur_res': cur_res, 'best_res': best_res})
    ...
    def prediction_loop(
        ...
        logger.info(f"***** Running {description} {self.args.run_name}*****")
        logger.info(f"  Num examples = {num_examples}")
