...
class BartAttention(nn.Module):
    ...
    def forward(
        ...
        if prefix_embed is not None:
            prefix_embed_k = prefix_embed[0].reshape(*proj_shape)
            prefix_embed_v = prefix_embed[1].reshape(*proj_shape)
            key_states = torch.cat([prefix_embed_k, key_states], dim=1)
            value_states = torch.cat([prefix_embed_v, value_states], dim=1)
            if attention_mask is not None:
                prefix_len = prefix_embed_k.size()[1]
                prefix_mask = torch.zeros((bsz, 1, tgt_len, prefix_len)).type_as(attention_mask)
...
class BartDecoderLayer(BartDecoderLayerAdaptersMixin, nn.Module):
    ...
    def forward(
            ...
            prefix_embed_self=None, prefix_embed_cross=None,
            **kwargs
    ):
        """
        Args:
        ...
        hidden_states, self_attn_weights, present_key_value = self.self_attn(
            ...
            layer_head_mask=layer_head_mask,
            output_attentions=output_attentions,
            prefix_embed=prefix_embed_self
        )
        hidden_states = F.dropout(hidden_states, p=self.dropout, training=self.training)
        hidden_states = self.attention_adapters.adapters_forward(hidden_states, residual, **kwargs)
        ...
        if encoder_hidden_states is not None:
            ...
            hidden_states, cross_attn_weights, cross_attn_present_key_value = self.encoder_attn(
                ...
                output_attentions=output_attentions,
                prefix_embed=prefix_embed_cross
            )
            hidden_states = F.dropout(hidden_states, p=self.dropout, training=self.training)
...
class BartEncoder(InvertibleAdaptersMixin, BartEncoderDecoderAdaptersMixin, BartPretrainedModel):
    ...
    def __init__(self, config: BartConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        self.add_enc_prefix = config.add_enc_prefix
        self.prefix_len = config.prefix_len
        self.mid_dim = config.mid_dim
        self.num_heads = config.encoder_attention_heads
        self.head_dim = embed_dim // self.num_heads

        if self.add_enc_prefix:
            self.prefix_enc_embed = nn.Embedding(self.prefix_len, config.d_model)
            # self.prefix_embed_MLP = nn.ModuleList(
            #     [nn.Sequential(
            #         nn.Linear(embed_dim, self.mid_dim),
            #         nn.Tanh(),
            #         nn.Linear(self.mid_dim, embed_dim)) for _ in range(2)])
            self.prefix_embed_MLP = nn.Sequential(
                nn.Linear(embed_dim, self.mid_dim),
                nn.Tanh(),
                nn.Linear(self.mid_dim, config.encoder_layers * 2 * embed_dim))
            self.prefix_ids = torch.arange(self.prefix_len).view(1, -1)
    ...
    def forward(
        ...
        if self.add_enc_prefix:
            ...
            prefix_ids = self.prefix_ids.repeat(bsz, 1).type_as(input_ids)
            # ones = torch.ones(bsz, self.prefix_len).type_as(attention_mask)
            # attention_mask = torch.cat([ones, attention_mask], dim=-1)
            prefix_embed_all = self.prefix_embed_MLP(self.prefix_enc_embed(prefix_ids) * self.embed_scale)
            prefix_embed_all = prefix_embed_all.view(bsz, -1, len(self.layers) * 2, self.num_heads, self.head_dim)
            prefix_embed_all = prefix_embed_all.permute([2, 0, 3, 1, 4]).split(2)
        ...
        for idx, encoder_layer in enumerate(self.layers):
            ...
            if self.training and (dropout_probability < self.layerdrop):  # skip the layer
                ...
                if getattr(self.config, "gradient_checkpointing", False) and self.training:
                    ...
                    #         # using shared raw embed now self.prefix_embed[0]
                    ...
                    #     hidden_states = torch.cat([prefix_embed, hidden_states], dim=1)
                    if self.add_enc_prefix and idx not in self.config.drop_first_prefix_layers_enc:
                        prefix_embed = prefix_embed_all[idx]
                    else:
                        prefix_embed = None
...
class BartDecoder(BartEncoderDecoderAdaptersMixin, BartPretrainedModel):
    ...
    def __init__(self, config: BartConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        self.add_dec_prefix = config.add_dec_prefix
        self.add_cross_prefix = config.add_cross_prefix
        self.prefix_len = config.prefix_len
        self.mid_dim = config.mid_dim
        self.num_heads = config.decoder_attention_heads
        self.head_dim = config.d_model // self.num_heads

        if self.add_dec_prefix:
            self.prefix_dec_embed = nn.Embedding(self.prefix_len, config.d_model)
            self.prefix_dec_embed_MLP = nn.Sequential(
                nn.Linear(config.d_model, self.mid_dim),
                nn.Tanh(),
                nn.Linear(self.mid_dim, config.decoder_layers * 2 * config.d_model))
            self.prefix_ids = torch.arange(self.prefix_len).view(1, -1)

        if self.add_cross_prefix:
            self.prefix_cross_embed = nn.Embedding(self.prefix_len, config.d_model)
            self.prefix_cross_embed_MLP = nn.Sequential(
                nn.Linear(config.d_model, self.mid_dim),
                nn.Tanh(),
                nn.Linear(self.mid_dim, config.decoder_layers * 2 * config.d_model))
            self.prefix_ids = torch.arange(self.prefix_len).view(1, -1)
    ...
    def forward(
        ...
        if self.add_dec_prefix:
            ...
            # ones = torch.ones(bsz, self.prefix_len).type_as(attention_mask)
            # attention_mask = torch.cat([ones, attention_mask], dim=-1)
            prefix_dec_embed_all = self.prefix_dec_embed_MLP(self.prefix_dec_embed(prefix_ids) * self.embed_scale)
            prefix_dec_embed_all = prefix_dec_embed_all.view(bsz, -1, len(self.layers) * 2, self.num_heads,
                                                             self.head_dim)
            prefix_dec_embed_all = prefix_dec_embed_all.permute([2, 0, 3, 1, 4]).split(2)
        ...
        if encoder_hidden_states is not None and encoder_attention_mask is not None:
            if self.add_cross_prefix:
                bsz = input_ids.shape[0]
                prefix_ids = self.prefix_ids.repeat(bsz, 1).type_as(input_ids)
                # ones = torch.ones(bsz, self.prefix_len).type_as(encoder_attention_mask)
                # encoder_attention_mask = torch.cat([ones, encoder_attention_mask], dim=-1)
                prefix_cross_embed_all = self.prefix_cross_embed_MLP(
                    self.prefix_cross_embed(prefix_ids) * self.embed_scale)
                prefix_cross_embed_all = prefix_cross_embed_all.view(bsz, -1, len(self.layers) * 2, self.num_heads,
                                                                     self.head_dim)
                prefix_cross_embed_all = prefix_cross_embed_all.permute([2, 0, 3, 1, 4]).split(2)
            # whether remove prefix?
            # if self.add_prefix:
            #     ones = torch.ones(encoder_attention_mask.shape[0], self.prefix_len).type_as(encoder_attention_mask)
            #     encoder_attention_mask = torch.cat([ones, encoder_attention_mask], dim=-1)
            # [bsz, seq_len] -> [bsz, 1, tgt_seq_len, src_seq_len]
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if getattr(self.config, "gradient_checkpointing", False) and self.training:
                ...
                if self.add_dec_prefix and idx not in self.config.drop_first_prefix_layers_dec:
                    prefix_embed_self = prefix_dec_embed_all[idx]
                else:
                    prefix_embed_self = None
                if self.add_cross_prefix and idx not in self.config.drop_first_prefix_layers_cross:
                    prefix_embed_cross = prefix_cross_embed_all[idx]
                else:
                    prefix_embed_cross = None
                layer_outputs = decoder_layer(
                    ...
                    output_attentions=output_attentions,
                    use_cache=use_cache,
                    prefix_embed_self=prefix_embed_self, prefix_embed_cross=prefix_embed_cross,
                    **kwargs,
                )
            hidden_states = layer_outputs[0]
            attention_mask = self.adjust_attention_mask_for_parallel(hidden_states, attention_mask)
